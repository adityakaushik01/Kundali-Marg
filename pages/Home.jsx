    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-gray-800 to-teal-900 text-white relative overflow-hidden">
      ...
      <section className="relative z-10 py-16 px-6">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { title: "Career & Success", desc: "Understand your true strengths and the path where you will thrive." },
              { title: "Love & Relationships", desc: "Decode compatibility, emotional patterns, and future possibilities." },
              { title: "Health & Life Purpose", desc: "Find deeper meaning and direction aligned with your destiny." },
            ].map((item, i) => (
              <div key={i} className="bg-white/5 border border-white/10 rounded-xl p-6 backdrop-blur-md hover:border-amber-400 duration-300 transition-all">
      ...
      <section className="relative z-10 py-24 px-6 bg-black/20 overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto">
          <p className="text-center text-amber-400 text-xs tracking-[0.4em] mb-3 uppercase">Powered by Technology</p>
          <h2 className="text-center text-3xl font-light tracking-wider mb-4">AI-POWERED ASTROLOGY</h2>
          <p className="text-center text-sm font-light opacity-50 max-w-xl mx-auto mb-16 leading-relaxed">
            Ancient Vedic wisdom meets modern artificial intelligence — for readings that are deeper, faster, and more personalised than ever before.
          </p>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="relative p-8 rounded-2xl border border-amber-500/15 bg-gradient-to-br from-amber-950/20 to-transparent group hover:border-amber-500/30 transition-all duration-300 flex flex-col justify-between min-h-[220px]">
              <div>
                ...
                <h3 className="text-lg font-normal tracking-wider mb-3">AI Kundali Interpretation</h3>
                <p className="text-sm font-light opacity-60 leading-relaxed">
                  Our platform uses advanced AI to interpret your Kundali with deep contextual understanding — delivering nuanced insights in seconds.
                </p>
              </div>
              <div className="mt-6 flex items-center gap-2">
                <div className="h-px flex-1 bg-gradient-to-r from-amber-500/30 to-transparent" />
                <span className="text-amber-400 text-xs tracking-widest opacity-70">INSTANT READINGS</span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-6">
              <div className="relative p-6 rounded-2xl border border-white/8 bg-white/3 hover:border-amber-500/25 transition-all duration-300 group">
                ...
                <h3 className="text-sm font-normal tracking-wider mb-2">Ask Anything</h3>
                <p className="text-xs font-light opacity-55 leading-relaxed">
                  Chat with our AI astrologer. Ask about your career, relationships, health, or any life question — and get answers grounded in your unique chart.
                </p>
              </div>
              <div className="relative p-6 rounded-2xl border border-white/8 bg-white/3 hover:border-amber-500/25 transition-all duration-300 group">
                {/* <span className="text-2xl mb-4 block">🔮</span> */}
                <h3 className="text-sm font-normal tracking-wider mb-2">Predictive Intelligence</h3>
                <p className="text-xs font-light opacity-55 leading-relaxed">
                  AI-driven Dasha and transit analysis identifies your most favourable windows — for decisions, relationships, and new beginnings.
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="relative z-10 py-24 px-6">
        <div className="max-w-4xl mx-auto">
          <p className="text-center text-amber-400 text-xs tracking-[0.4em] mb-3 uppercase">The Process</p>
          <h2 className="text-center text-3xl font-light tracking-wider mb-20">HOW IT WORKS</h2>
          ...
          <div className="relative flex flex-col md:flex-row items-center justify-between gap-12">
            ...
            {[
              { num: "01", title: "Enter Details", desc: "Provide your birth date, time, and place of birth." },
              { num: "02", title: "Planetary Analysis", desc: "We map every planet's position at your exact moment of birth." },
              { num: "03", title: "Your Insights", desc: "Receive a detailed, personalised Kundali reading instantly." },
            ].map((step, i) => (
              <div key={i} className="flex-1 flex flex-col items-center text-center group">
                <div className="w-14 h-14 rounded-full border border-amber-500/40 flex items-center justify-center mb-6 group-hover:border-amber-400 group-hover:bg-amber-500/10 transition-all duration-300 relative bg-slate-900">
                  <span className="text-amber-400 text-sm font-light tracking-widest">{step.num}</span>
                </div>
                <h3 className="text-base font-normal tracking-wider mb-3">{step.title}</h3>
                <p className="text-sm font-light opacity-60 leading-relaxed max-w-[180px]">{step.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="relative z-10 py-24 px-6">
        <div className="max-w-5xl mx-auto">
          <p className="text-center text-amber-400 text-xs tracking-[0.4em] mb-3 uppercase">What&apos;s Included</p>
          <h2 className="text-center text-3xl font-light tracking-wider mb-16">POWERFUL FEATURES</h2>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { icon: "🪐", label: "Kundali Charts", desc: "Full birth chart with all 12 houses" },
              { icon: "⚡", label: "Dosha Detection", desc: "Identify Mangal, Kaal Sarp & more" },
              { icon: "✦", label: "Yoga Analysis", desc: "Auspicious planetary combinations" },
              { icon: "⏳", label: "Dasha Predictions", desc: "Period-wise life forecasts" },
            ].map((f, i) => (
              <div key={i} className="group p-6 rounded-2xl border border-white/8 bg-white/3 hover:bg-amber-500/5 hover:border-amber-500/30 transition-all duration-300 text-center flex flex-col items-center gap-3">
      ...
      <section className="relative z-10 py-28 px-6 text-center overflow-hidden">
        ...
        <p className="relative text-amber-400 text-xs tracking-[0.4em] mb-4 uppercase">Begin Your Journey</p>
        <h2 className="relative text-4xl md:text-5xl font-light tracking-wider mb-5">
          Your Destiny<br />Is Waiting
        </h2>
      ...
      <section className="relative z-10 py-16 px-6 border-t border-white/5 bg-black/20">
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-xs font-light opacity-30 leading-relaxed mb-10 italic">
            The information provided, whether free or paid, must not be considered as indicating an opinion in the
            realms of psychology, economy/finance, medicine, legal matters, real estate or others, and must in no
            case be used as a basis for decision making, investment, or medical advice. Judgments are free,
            not-for-sale opinions that may not yield identical results for everybody. Please also read the Terms of Use.
          </p>

          <div className="flex flex-wrap justify-center gap-6 text-xs font-light opacity-40 mb-2">
            {["Home", "Terms of Service", "Privacy Policy", "Ethical Rules", "GDPR", "Cookie Policy"].map((l) => (
              <a key={l} href="#" className="hover:text-amber-400 hover:opacity-100 transition-all duration-200">{l}</a>
            ))}
          </div>

          <p className="text-xs opacity-20 mt-8 tracking-widest">© {new Date().getFullYear()} KUNDALI MARG</p>
        </div>
      </section>
