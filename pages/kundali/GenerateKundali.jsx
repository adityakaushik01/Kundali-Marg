  ...
  components: {
        ...
        root: {
          "& .MuiOutlinedInput-root": {
            ...
            "&.Mui-focused": {
              ...
              "& .MuiOutlinedInput-notchedOutline": { borderColor:"#f59e0b", borderWidth:"1.5px" },
            },
            "& .MuiOutlinedInput-notchedOutline": { borderColor:"rgba(255,255,255,0.12)" },
          },
          "& .MuiInputLabel-root": { color:"rgba(255,255,255,0.4)", "&.Mui-focused":{ color:"#f59e0b" } },
          "& .MuiInputBase-input": { color:"#ffffff", cursor:"pointer", fontWeight:300 },
    ...
    MuiClock: {
      styleOverrides: {
        ...
        clock:    { backgroundColor:"rgba(255,255,255,0.03)", border:"1px solid rgba(245,158,11,0.2)" },
        amButton: { color:"#ffffff", "&.Mui-selected":{ backgroundColor:"#f59e0b", color:"#000" } },
        pmButton: { color:"#ffffff", "&.Mui-selected":{ backgroundColor:"#f59e0b", color:"#000" } },
      },
    },
    MuiClockNumber: {
      styleOverrides: {
        root: { color:"rgba(255,255,255,0.8)", "&.Mui-selected":{ backgroundColor:"#f59e0b", color:"#000" } },
      },
    },
    MuiClockPointer: {
      styleOverrides: {
    ...
    MuiDialogActions: {
      styleOverrides: {
        root: { "& .MuiButton-root":{ color:"#f59e0b", "&:hover":{ backgroundColor:"rgba(245,158,11,0.1)" } } },
      },
    },
  },
});
...
const GenerateKundali = () => {
  ...
  const validateForm = () => {
    if (!formData.name.trim())             { setError("Please enter the full name"); return false; }
    if (!formData.birthDate)               { setError("Please select a birth date"); return false; }
    if (!formData.birthTime)               { setError("Please select a birth time"); return false; }
    if (!formData.address.trim())          { setError("Please enter the birth place"); return false; }
        ...
        <main className="flex-1 px-5 md:px-7 py-8 relative z-10 w-full max-w-5xl mx-auto">
          ...
          <div className="mb-8">
            ...
            <h1 className="text-2xl md:text-3xl font-light tracking-wider" style={{ color:r(0.92) }}>
              Generate <span style={{ color:COLORS.amber.text }}>Janam Kundali</span>
            </h1>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                  ...
                  {error && (
                    <div className="flex items-start gap-3 p-4 rounded-xl"
                      style={{ background:COLORS.rose.bg, border:`1px solid ${COLORS.rose.border}` }}>
                      <span style={{ fontSize:"14px", flexShrink:0 }}>⚠️</span>
                      <p className="text-sm font-light" style={{ color:COLORS.rose.text }}>{error}</p>
                    </div>
                  )}

                  {/* Full Name */}
                  <div>
                    <label className="block text-xs font-light tracking-[0.2em] uppercase mb-2.5"
                      style={{ color:r(0.4) }}>
                      Full Name <span style={{ color:COLORS.amber.text }}>*</span>
                    </label>
                    <input
                      type="text"
                      value={formData.name}
                      ...
                      style={inputStyle}
                      onFocus={e => { e.target.style.borderColor=COLORS.amber.border; e.target.style.background="rgba(255,255,255,0.07)"; }}
                  ...
                  <div>
                    <label className="block text-xs font-light tracking-[0.2em] uppercase mb-2.5"
                      style={{ color:r(0.4) }}>
                      Date & Time of Birth <span style={{ color:COLORS.amber.text }}>*</span>
                    </label>
                          ...
                          <DatePicker
                            ...
                            slotProps={{
                              textField: {
                                ...
                                InputProps: {
                                  style:    { fontSize:"14px", cursor:"pointer", fontWeight:300 },
                                  readOnly: true,
                                },
                              },
                            }}
                  ...
                  {/* Place of Birth */}
                  <div>
                    <label className="block text-xs font-light tracking-[0.2em] uppercase mb-1"
                      style={{ color:r(0.4) }}>
                      Place of Birth <span style={{ color:COLORS.amber.text }}>*</span>
                    </label>
                    <p className="text-xs font-light mb-2.5" style={{ color:r(0.28) }}>
                        ...
                        <div className="absolute top-full left-0 right-0 mt-2 rounded-xl overflow-hidden z-50"
                          ...
                          {suggestions.map((item, idx) => (
                            <div key={idx}
                              ...
                              style={{ borderBottom: idx<suggestions.length-1 ? `1px solid ${r(0.06)}` : "none" }}
                              onMouseEnter={e => e.currentTarget.style.background=COLORS.amber.bg}
                              onMouseLeave={e => e.currentTarget.style.background="transparent"}>
                              <div className="flex items-start gap-3">
                                <span style={{ fontSize:"13px", marginTop:"2px", flexShrink:0 }}>📍</span>
                                <div>
                                  <p className="text-sm font-light" style={{ color:r(0.82) }}>
                                    {item.properties.formatted}
                                  </p>
                                  <p className="text-xs font-light mt-0.5" style={{ color:r(0.3) }}>
                                    {parseFloat(item.properties.lat).toFixed(4)}°, {parseFloat(item.properties.lon).toFixed(4)}°
                                  </p>
                                </div>
                              </div>
                            </div>
                    ...
                    {formData.latitude && formData.longitude && (
                      <div className="mt-3 flex items-center gap-2 px-4 py-2.5 rounded-xl"
                        style={{ background:COLORS.teal.bg, border:`1px solid ${COLORS.teal.border}` }}>
                          <div className="flex items-center gap-2"></div>
                        <span style={{ fontSize:"12px", color:COLORS.teal.text }}>✓</span>
                        <p className="text-xs font-light" style={{ color:COLORS.teal.text }}>
                          {formData.address}
                        </p>
                      </div>
                    )}
                  ...
                  <button
                    ...
                    style={{
                      ...
                      color:      "#fff",
                      boxShadow:  loading ? "none" : `0 4px 24px ${COLORS.amber.glow}`,
                    }}>
                    {loading ? (
            ...
            <div className="space-y-4">
              ...
              <div style={glass(COLORS.amber.border)}>
                <div className="px-5 py-4" style={{ borderBottom:`1px solid ${r(0.07)}` }}>
                  <p className="text-[10px] tracking-[0.3em] uppercase mb-0.5" style={{ color:r(0.3) }}>Important</p>
                  <h3 className="text-sm font-light tracking-wider" style={{ color:r(0.85) }}>Why accuracy matters</h3>
                </div>
                <div className="p-5 space-y-4">
                  {[
                    { icon:"📅", title:"Date",  desc:"Determines exact planetary positions" },
                    { icon:"⏰", title:"Time",  desc:"Critical for Lagna (ascendant) calculation" },
                    { icon:"📍", title:"Place", desc:"Affects all 12 house placements" },
                  ].map(item => (
                    <div key={item.title} className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-xs font-light tracking-wide mb-0.5" style={{ color:r(0.75) }}>{item.title}</p>
                        <p className="text-xs font-light leading-relaxed" style={{ color:r(0.38) }}>{item.desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div style={glass()}>
                <div className="px-5 py-4" style={{ borderBottom:`1px solid ${r(0.07)}` }}>
                  <p className="text-[10px] tracking-[0.3em] uppercase mb-0.5" style={{ color:r(0.3) }}>Output</p>
                  <h3 className="text-sm font-light tracking-wider" style={{ color:r(0.85) }}>What you'll receive</h3>
                </div>
                <div className="p-5 space-y-2.5">
                  {[
                    ...
                    <div key={i} className="flex items-center gap-2.5">
                      ...
                      <span className="text-xs font-light" style={{ color:r(0.55) }}>{item}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={glass()}>
                <div className="p-5 flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-xs font-light tracking-wide mb-1" style={{ color:r(0.65) }}>
                      Your data is private
                    </p>
                    <p className="text-xs font-light leading-relaxed" style={{ color:r(0.32) }}>
